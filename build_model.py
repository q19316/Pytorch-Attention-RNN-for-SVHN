...
class EncoderCNN(nn.Module):
    """
    CNN to extract visual features from the input image.
    """
    def __init__(self, hidden_size, drop_p):
        """
        Args:
    ...
    def forward(self, xs):
        ...
        Returns:
            ftrs (torch.FloatTensor, [batch_size, 512, 4, 8]): EncoderCNN's output feature map.
        """
        ftrs = self.cnn(xs)
        ftrs = self.drop(ftrs)
        return ftrs
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, encoder_ftrs, sentences, test):
        ...
        Args:
            encoder_ftrs (torch.FloatTensor, [batch_size, 512, 4, 8]): EncoderCNN's output feature map.
        ...
        """
        states = encoder_ftrs.view(-1, 512, 32).transpose(1,2)       # [batch_size, 32, 512]
        ...
        if test:
            ...
            while (time_step < 100):
                ...
                attns, attn_weights = self.apply_attn(states, h[-1])   # [1, 512], [1, 32]
    ...
    def apply_attn(self, source_states, target_states):
        ...
        Args:
            source_states (torch.FloatTensor, [batch_size, 32, 512]):
        ...
        Returns:
            ...
            attn_weights (torch.FloatTensor, [batch_size, 32]): The attention alignment weights.
        """
        # A two-layer network used for project every pair of [source_state, target_state].
        attns = self.attn_W(source_states) + self.attn_U(target_states).unsqueeze(1)   # [batch_size, 32, hidden_size]
        attns = self.attn_v(F.relu(attns)).squeeze(2)                   # [batch_size, 32]
        attns = F.softmax(attns, dim=-1)                                # [batch_size, 32]
...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, target_size, hidden_size, drop_p=0.):
        ...
        self.encoder = EncoderCNN(hidden_size, drop_p)
